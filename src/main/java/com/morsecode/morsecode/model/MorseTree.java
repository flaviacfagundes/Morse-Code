...
public class MorseTree {
    ...
    public MorseTree() { this.root = null; }
    ...
    public void insertRecursiveDirection(Node node, char letter, String morseCode, int index, char symbol) {
        if (symbol == '.') {
            // Ponto vai à esquerda
            if (node.getLeft() == null) { node.setLeft(new Node(' ', "")); /* Nó temporário */ }
            insertRecursive(node.getLeft(), letter, morseCode, index + 1);
        } else if (symbol == '-') {
            // Traço vai à direita
            if (node.getRight() == null) { node.setRight(new Node(' ', "")); /* Nó temporário */ }
     ...
     */
    public char decodeChar(String morseCode) { return decodeCharRecursive(root, morseCode, 0); }
    ...
    private char decodeCharRecursive(Node node, String morseCode, int index) {
        if (node == null) { return '?'; /* Caractere não encontrado */ }

        if (index >= morseCode.length()) { return node.getLetter(); /* Chegou ao final, retorna o caractere do nó */ }

        char symbol = morseCode.charAt(index);

        if (symbol == '.') { return decodeCharRecursive(node.getLeft(), morseCode, index + 1); }
        else if (symbol == '-') { return decodeCharRecursive(node.getRight(), morseCode, index + 1); }
    ...
    public String decodeWord(String morseWord) {
        if (morseWord == null || morseWord.trim().isEmpty()) { return ""; }
    ...
    private String encodeCharRecursive(Node node, char letter, String currentPath) {
        ...
        if (node.getLetter() == letter && !node.getMorseCode().isEmpty()) { return node.getMorseCode(); }
    ...
    public void encodeWordResultCheckSpace(char letter, String upperWord, StringBuilder result, int i) {
        if (letter != ' ') {
            String morse = encodeChar(letter);
            if (!morse.isEmpty() && i < upperWord.length() - 1 && upperWord.charAt(i + 1) != ' ') {
                result.append(morse);
                result.append(" ");
            } else if (!morse.isEmpty()) { result.append(morse); }
        } else { result.append(" / "); }
    }

    public StringBuilder encodeWordResult(String upperWord) {
        StringBuilder result = new StringBuilder();

        for (int i = 0; i < upperWord.length(); i++) {
            char letter = upperWord.charAt(i);
            encodeWordResultCheckSpace(letter, upperWord, result, i);
        }
        return result;
    }
    ...
    public String encodeWord(String word) {
        ...
        return encodeWordResult(upperWord).toString().trim();
    }
    ...
    private void printTreeRecursive(Node node, String prefix, boolean isLast) {
        ...
        if (node.getLetter() != ' ' && !node.getMorseCode().isEmpty()) { System.out.println("'" + node.getLetter() + "' (" + node.getMorseCode() + ")"); }
        else { System.out.println("[nó interno]"); }

        if (node.getLeft() != null || node.getRight() != null) {
            String newPrefix = prefix + (isLast ? "    " : "│   ");

            if (node.getRight() != null) { printTreeRecursive(node.getLeft(), newPrefix, node.getRight() == null); }
            if (node.getRight() != null) { printTreeRecursive(node.getRight(), newPrefix, true); }
        }
    }
    ...
    private void getTreeAsStringRecursive(Node node, String prefix, boolean isLast, StringBuilder sb) {
        ...
        if (node.getLetter() != ' ' && !node.getMorseCode().isEmpty()) { sb.append("'").append(node.getLetter()).append("' (").append(node.getMorseCode()).append(")\n"); }
        else { sb.append("[nó interno]\n"); }

        if (node.getLeft() != null || node.getRight() != null) {
            String newPrefix = prefix + (isLast ? "    " : "│   ");

            if (node.getLeft() != null) { getTreeAsStringRecursive(node.getLeft(), newPrefix, node.getRight() == null, sb); }
            if (node.getRight() != null) { getTreeAsStringRecursive(node.getRight(), newPrefix, true, sb); }
        }
    }
}
